...
public class ProductsController : BaseFestivalController<ProductsController>
{
    ...
    [HttpGet("GetAll")]
    public async Task<IActionResult> GetAll(int pageNumber, int pageSize, string searchString, int? festivalId,string orderBy = null)
    {
        var products = await Mediator.Send(new GetAllProductsQuery(pageNumber, pageSize, searchString, orderBy,festivalId));
        return Ok(products);
    }
