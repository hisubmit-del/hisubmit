...
namespace HiSubmit.Application.Features.Dashboards.Queries.GetData
{
    ...
    internal class GetDashboardDataQueryHandler : IRequestHandler<GetDashboardDataQuery, Result<DashboardDataResponse>>
    {
        ...
        public async Task<Result<DashboardDataResponse>> Handle(GetDashboardDataQuery query, CancellationToken cancellationToken)
        {
            var response = new DashboardDataResponse
            {
                ...
                BrandCount = await _unitOfWork.Repository<ArtCategory>().Entities.CountAsync(cancellationToken),
            ...
            for (int i = 1; i <= 12; i++)
            {
                ...
                brandsFigure[i - 1] = await _unitOfWork.Repository<ArtCategory>().Entities.Where(x => x.CreatedOn >= filterStartDate && x.CreatedOn <= filterEndDate).CountAsync(cancellationToken);
            ...
            response.DataEnterBarChart.Add(new ChartSeries { Name = _localizer["ArtCategory"], Data = brandsFigure });
