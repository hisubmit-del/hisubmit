...
public class FestivalSubUsersController(IRoleService roleService, IUserService userService)
    : BaseFestivalController<FestivalSubUsersController>
{
    /// <summary>
    ...
    /// <returns></returns>
    [HttpPost("SaveRole")]
    public async Task<IActionResult> CreateFestivalRoles(AddEditFestivalRoleCommand command,int festivalId)
    {
    ...
    {
        return Ok(await roleService.GetAllPermissionsAsync(roleId, PermissionType.Festival));
    }
    ...
    {
        var response = await userService.GetRolesAsync(userId, fId);
        return Ok(response);
    }
