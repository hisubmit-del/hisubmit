...
using HiSubmit.Application.Interfaces.Services.Storage;
using Hisubmit.Client.SharedModels.Constants.Storage;

namespace HiSubmit.Server.Managers.Preferences;
...
public class ServerPreferenceManager : IServerPreferenceManager
{
    ...
    {
        return await _serverStorageService.GetItemAsync<ServerPreference>
            (StorageConstants.Server.Preference) ?? new ServerPreference();
    }

    public async Task SetPreference(IPreference preference)
    {
        await _serverStorageService.SetItemAsync
            (StorageConstants.Server.Preference, preference as ServerPreference);
    }
}