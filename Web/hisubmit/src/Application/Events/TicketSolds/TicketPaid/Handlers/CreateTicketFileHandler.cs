...
public class CreateTicketFileHandler : INotificationHandler<PaidTicketEvent>
{
    ...
    public CreateTicketFileHandler
    ...
    public async Task Handle(PaidTicketEvent notification, CancellationToken cancellationToken)
    {
    ...
    public async Task CreateFile(PaidTicketEvent notification, CancellationToken cancellationToken)
    {
        ...
        var content = await _renderViewService
            .RenderViewToStringAsync("Tickets", model, "Tickets");
        var pdfFileByteArray = await _pdfGeneratorl
            .GenerateFile(new PdfGeneratorRequest()
        {
            Content = content,
        ...
        var mailRequest = new MailRequest()
        {
            ...
            Attachments = new List<MailAttachment>()
                { new MailAttachment() { File = soldTicket.PdfFile, Name =$"{soldTicket.Ticket.Title}.pdf" } }
        };
