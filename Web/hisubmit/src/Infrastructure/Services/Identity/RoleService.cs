...
namespace HiSubmit.Infrastructure.Services.Identity
{
    public class RoleService : IRoleService
    {
        ...
        public async Task<Result<string>> DeleteAsync(string id)
        {
            ...
            if (existingRole.Name != RoleConstants.AdministratorRole && existingRole.Name != RoleConstants.ArtistRole)
            {
        ...
        public async Task<Result<List<RoleResponse>>> GetAllAsync(int? festivalId=null)
        {
            var query = _roleManager.Roles;
            query = query.Where(x => x.FestivalId == festivalId);
        ...
        public async Task<Result<PermissionResponse>> GetAllPermissionsAsync(string roleId, PermissionType? permissionType)
        {
            var model = new PermissionResponse();
            var allPermissions = GetAllPermissions(permissionType);
        ...
        private List<RoleClaimResponse> GetAllPermissions(PermissionType? permissionType)
        {
            var allPermissions = new List<RoleClaimResponse>();

            #region GetPermissions

            allPermissions.GetAllPermissions(permissionType);
        ...
        public async Task<Result<string>> SaveAsync(RoleRequest request)
        {
            ...
            {
                var existingRole = await _roleManager.Roles.FirstOrDefaultAsync(p=>p.Name==request.Name
                ...
                var response = await _roleManager.CreateAsync(new BlazorHeroRole(request.Name,request.FestivalId, request.Description));
            ...
            else
            {
                ...
                existingRole.FestivalId = request.FestivalId;
