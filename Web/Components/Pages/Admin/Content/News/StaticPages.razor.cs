...
namespace Web.Components.Pages.Admin.Content.News;

public partial class StaticPages
{
    #region Inject
    [Inject] public IStaticPageManager NewManager { get; set; }
    ...
    private async Task<TableData<GetAllStaticPageResponse>> ServerReload(TableState state ,System.Threading.CancellationToken  token)
    {
        _loaded = false;
    ...
    private void Add()
    {
        _navigationManager.NavigateTo("/admin/static-page");
    }

    private async Task Edit(int id)
    {
        _navigationManager.NavigateTo($"/admin/static-page/{id}");
    }
    ...
    private async Task Delete(int id, string title)
    {
        ...
        if (!result.Canceled)
        {
            var response = await NewManager.DeleteAsync(new DeleteStaticPageCommand()
            {
                Id = id
            });
            if (response.Succeeded)
