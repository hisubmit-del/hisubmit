...
public class UserController(IUserService userService, IMediator mediator) : ControllerBase
{
    ...
    {
        var userRoles = await userService.GetRolesAsync(id);
        return Ok(userRoles);
    }
    ...
    {
        return Ok(await userService.UpdateRolesAsync(request));
    }
    ...
    {
        return Ok(await userService.ConfirmEmailAsync(userId, code));
    }

    /// <summary>
    /// Toggle User Enable (Activate and Deactivate)
    /// </summary>
    /// <param name="request"></param>
    /// <returns>Enable 200 OK</returns>
    ...
    {
        return Ok(await userService.ToggleUserStatusAsync(request));
    }
    ...
    {
        var origin = Request.Headers["origin"];
        return Ok(await userService.ForgotPasswordAsync(request, origin));
    }
    ...
    {
        return Ok(await userService.ResetPasswordAsync(request));
    }
    ...
    {
        var data = await userService.ExportToExcelAsync(searchString);
        return Ok(data);
    }
}