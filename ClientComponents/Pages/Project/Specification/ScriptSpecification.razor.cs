...
namespace ClientComponents.Pages.Project.Specification;

public partial class ScriptSpecification
{
    #region Inject
    ...
    private AddEditScriptSpecificationCommand Model { get; set; } = new();
    ...
    private async Task LoadSpecification()
    {
        var response = await ProjectSpecificationManager
            .GetScriptSpecification(new GetScriptSpecificationDetailQuery() { ProjectId = ProjectId });
        if (response.Succeeded)
        {
            Model = Mapper.Map<AddEditScriptSpecificationCommand>(response.Data);
    ...
    public async Task<bool> SaveAsync()
    {
        ...
        if (_validate)
        {
            ...
            var response = await ProjectSpecificationManager.UpdateScriptSpecification(Model);
            _processing = false;
            if (response.Succeeded)
            {
