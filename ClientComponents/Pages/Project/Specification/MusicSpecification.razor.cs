...
public partial class MusicSpecification
{
    #region Inject
    ...
    private AddEditMusicSpecificationCommand Model { get; set; } = new();
    ...
    private async Task LoadSpecification()
    {
        var response = await ProjectSpecificationManager
            .GetMusicSpecification(new GetMusicSpecificationDetailQuery { ProjectId = ProjectId });
        if (response.Succeeded)
        {
            Model = Mapper.Map<AddEditMusicSpecificationCommand>(response.Data);
    ...
    public async Task<bool> SaveAsync()
    {
        ...
        if (_validate)
        {
            ...
            var response = await ProjectSpecificationManager.UpdateMusicSpecification(Model);
            _processing = false;
            if (response.Succeeded)
            {
