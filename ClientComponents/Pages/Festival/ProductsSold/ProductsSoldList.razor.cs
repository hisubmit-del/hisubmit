...
namespace ClientComponents.Pages.Festival.ProductsSold;

public partial class ProductsSoldList
{
     #region Inject

    [Inject] private IFestivalSoldProductManager FestivalSoldTicketManager { get; set; }
    ...
    private async Task LoadData(int pageNumber, int pageSize, TableState state, GetAllSoldProductQuery query)
    {
        ...
        var response = await FestivalSoldTicketManager.GetAllAsync(query);
    ...
    private void ShowDetail(int productSoldId)
    {
        ...
        {
            {nameof(ProductSoldDetailDialog.FestivalId),SelectedFestivalId},
